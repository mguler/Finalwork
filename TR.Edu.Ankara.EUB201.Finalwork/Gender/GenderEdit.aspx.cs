...
{
    public partial class GenderEdit : System.Web.UI.Page
    {
        private readonly GenderService _genderService;
        public GenderEdit(GenderService genderService)
        {
        ...
        {
            var data = _genderService.GetDetail(id);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var result = _genderService.Save(id, txtName.Text, checkIsActive.Checked);
        ...
        {
            Response.Redirect("GenderEdit.aspx");
        }
        protected void btnSil_Click(object sender, EventArgs e)
        {
            try
            {
                var id = 0;
                int.TryParse(recordId.Value, out id);
                var result = _genderService.Delete(id);
                if(!result) throw new CustomApplicationException("işleminiz yapılamadı. Daha sonra tekrar deneyiniz");
                Response.Redirect("/Successful.aspx?url=/Gender/GenderList.aspx");
            }
            catch (CustomApplicationException ex)
            {
                errorText.Visible = true;
